...
def filter_runs(runs, max_recent=10):
    """Always keep the run19 baseline; add the most recent `max_recent` runs."""
    run19 = [r for r in runs if "run19" in r["name"]]
    others = [r for r in runs if "run19" not in r["name"]]
    # `runs` is already sorted oldest-first by mtime — take the tail for "recent"
    recent = others[-max_recent:]
    selected = run19 + recent
    # Re-sort oldest-first so run19 stays at the top
    selected.sort(key=lambda r: os.path.getmtime(r["path"]))
    return selected


if __name__ == "__main__":
    runs = discover_runs()
    runs = filter_runs(runs, max_recent=10)
    print(f"Showing {len(runs)} runs with tracking images:")
    for r in runs:
        epochs = r["epochs"]
