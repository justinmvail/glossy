...
def optimize_diffvg(font_path: str, char: str, canvas_size: int,
    ...
    """Optimize strokes using DiffVG differentiable rendering in Docker.

    DiffVG (Differentiable Vector Graphics) enables gradient-based optimization
    of vector stroke parameters by providing differentiable rasterization. This
    allows using gradient descent to minimize the difference between rendered
    strokes and the target glyph.

    This function delegates the actual optimization to a Docker container running
    DiffVG, communicating via the diffvg_docker interface.

    Args:
        font_path: Path to the target font file.
        char: The character to optimize strokes for.
        canvas_size: Size of the square canvas in pixels.
        diffvg_docker: Docker interface object with an optimize() method. If None,
            the function returns None immediately.
        template_to_strokes_fn: Callable that extracts initial template strokes.
            Signature: template_to_strokes_fn(font_path, char, canvas_size)
                -> strokes or (strokes, ...) tuple or None
        render_glyph_mask_fn: Callable that renders the glyph as a binary mask.
            Signature: render_glyph_mask_fn(font_path, char, canvas_size)
                -> numpy array or None
        logger_ref: Logger object for warning messages.
        num_iterations: Number of optimization iterations for DiffVG. Higher values
            may produce better results but take longer. Defaults to 500.
        stroke_width: Width of strokes in pixels for rendering. Defaults to 8.0.
        timeout: Maximum time in seconds to wait for Docker optimization.
            Defaults to 300 (5 minutes).

    Returns:
        A tuple of (strokes, score, mask, glyph_bbox) in the same format as
        optimize_affine:
            - strokes: List of optimized strokes as [[x, y], ...]
            - score: DiffVG optimization score (higher is better)
            - mask: The binary glyph mask
            - glyph_bbox: (x_min, y_min, x_max, y_max) bounding box

        Returns None if:
            - diffvg_docker is None
            - No template strokes are available
            - Docker optimization returns an error
            - Optimized strokes are empty or have non-positive score
            - Glyph mask cannot be rendered

    Note:
        DiffVG optimization typically produces smoother results than affine
        optimization but requires more computational resources and a Docker
        environment with GPU support for best performance.
    """
